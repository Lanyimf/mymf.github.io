---
title: 土地開發評估系統
---

<div class="hero">
  <h1>土地開發評估系統</h1>
  <p>整合 342 筆場址資料、72 條評估規則與 44 部法規，協助你快速查找與評估土地開發可行性。</p>
</div>

<div class="stat-strip">
  <div class="stat-item"><span class="stat-num">342</span><span class="stat-label">場址資料</span></div>
  <div class="stat-item"><span class="stat-num">72</span><span class="stat-label">評估規則</span></div>
  <div class="stat-item"><span class="stat-num">44</span><span class="stat-label">完整法規</span></div>
  <div class="stat-item"><span class="stat-num">5</span><span class="stat-label">評估案例</span></div>
</div>

<div class="feature-cards">
  <a class="feature-card" href="lands">
    <div class="feature-title">場址資料</div>
    <p>瀏覽全部 342 筆場址，可依縣市篩選或輸入關鍵字搜尋。</p>
  </a>
  <a class="feature-card" href="rules">
    <div class="feature-title">地目用地類別</div>
    <p>從想要的開發用途反查，哪些場址符合條件。</p>
  </a>
  <a class="feature-card" href="用地評估">
    <div class="feature-title">用地評估</div>
    <p>選一個地點，自動跑過全部 72 條規則，列出符合／不符合／資料不足。</p>
  </a>
</div>

## 全台場址地圖

<div class="all-lands-map-host"></div>

## 快速查詢

<div class="rule-chips">

[[rules/EE|EE 農牧用地]] [[rules/EB|EB 乙種建築用地]] [[rules/ED|ED 丁種建築用地]] [[rules/EH|EH 水利用地]] [[rules/EG|EG 遊憩用地]] [[rules/EP|EP 特定目的事業用地]] [[rules/EN|EN 國土保育用地]]

</div>

## 核心法規

- [[laws/非都市土地使用管制規則]]
- [[laws/農業發展條例]]
- [[laws/國土計畫法]]
- [[laws/土壤及地下水污染整治法]]
- [[laws/建築法]]
- [[laws/環境影響評估法]]

## 法規覆蓋狀態

- 44 部法規全部有完整條文
- 44 部全部被 rules/ 引用
- 72 個評估代號全部有硬性門檻
- 無斷連連結、無孤島法規
